...
config = {
    ...
    'feeRate': 0.001,
    'feeLimit': 5,
    'strategyName': 'Strategy_weightVarmin_selcted',  # strategy name
    "logfile": "maday",
    'dealByVolume': True,
    "memorySize": 5,
    'assetType': 'STOCK'
...
def getOptWeight(sdk, stockCodeList, FactorNames, exposurePeriod, bencmarkIndexCode):
    ...
    q = matrix(np.zeros((1, len(stockCodeList))), (len(stockCodeList), 1))
    ...
    G = -1 * d
    h = matrix(np.zeros((len(stockCodeList), 1)))
    # solving the QP
