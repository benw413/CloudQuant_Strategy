...
'''this strategy has certain good thought, but no close to real trading enviorment'''
...
config = {
    ...
    'feeRate': 0.001,
    'feeLimit': 5,
    'strategyName': 'LZ_TEST_BPSSB_HA_001035',  # strategy name
    "logfile": "maday",
    'dealByVolume': True,
    "memorySize": 5,
    'assetType': 'STOCK'
...
def strategy(sdk):
    ...
    global dayCounter
    delayed = sdk.getGlobal("DELAYSELLS")
    sellandUpdateDelayed(sdk, list(delayed))
    if (dayCounter - 1) % PERIOD == 0:
        ...
        if stockToSell:
            ...
            if not flag:
                currentHolding = [i.code for i in sdk.getPositions()]
                unsuccessful = sellStockAndShowUnsuccess(sdk, currentHolding, quotes)
                sdk.setGlobal("DELAYSELLS", sdk.getGlobal("DELAYSELLS") | set(unsuccessful))
        if stockToBuy:
            if flag:
...
def constrianDelayedSell(stockToBuy, stockToSell, delayedSell):
    ...
    return sell
def sellandUpdateDelayed(sdk, delayed, stockToBuy=[], stockToSell=[]):
    if delayed:
